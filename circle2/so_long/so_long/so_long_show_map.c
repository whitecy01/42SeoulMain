...
/*   Updated: 2023/09/05 02:43:10 by jaeyojun         ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
void	categorize_case(t_game *game, int x, int y, int index)
{
	if (game->map_line[index] == '0')
		put_img(game, y, x, "./textures/back_ground.xpm");
	else if (game->map_line[index] == '1')
	{
		put_img(game, y, x, "./textures/back_ground.xpm");
		put_img(game, y, x, "./textures/tree.xpm");
	}
	else if (game->map_line[index] == 'E')
	{
		put_img(game, y, x, "./textures/back_ground.xpm");
		if (game->c_check == game->c)
			put_img(game, y, x, "./textures/door_open.xpm");
		else
			put_img(game, y, x, "./textures/door.xpm");
	}
	else if (game->map_line[index] == 'C')
	{
		put_img(game, y, x, "./textures/back_ground.xpm");
		put_img(game, y, x, "./textures/eat.xpm");
	}
	else if (game->map_line[index] == 'P')
	{
		put_img(game, y, x, "./textures/back_ground.xpm");
		put_img(game, y, x, "./textures/player.xpm");
	}
}
